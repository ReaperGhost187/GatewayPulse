...
public static class MonitorApplication
{
    public static async Task<int> RunAsync(MonitorOptions options, CancellationToken cancellationToken)
    {
        ...
        RawCaptureWriter? capture = null;
        if (options.CaptureRaw && options.Mode != MonitorMode.Mock)
        {
            capture = new RawCaptureWriter(options.LogsPath);
            logger.Info($"Raw P capture enabled → {capture.FilePath}");
        }

        IRfMonitor monitor = options.Mode == MonitorMode.Mock
            ? new MockRfProvider()
            : new TelePostLp100Provider(
                options.Port,
                options.AutoDetect,
                options.BaudRate,
        ...
        try
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    if (capture?.IsFull == true)
                        logger.Info("Raw P capture file reached size limit; further frames discarded.");
